...
export function PfpAndGenres({ profilePicture, artistGenres, updateFields, errors }: PfpAndGenresProps) {
	...
	const handleGenreClicked = (event: React.MouseEvent<HTMLDivElement, MouseEvent>, newGenre: Genre) => {
		...
		if(artistGenres.includes(newGenre)){
			updateFields({artistGenres: artistGenres.filter(genre => genre !== newGenre)});
		} else {
			updateFields({artistGenres: [...artistGenres, newGenre]});
		}
	};
	return (
				...
				<div className="flex flex-col gap-2 w-full">
					...
					<div className="flex flex-row gap-2 flex-wrap">
						{sortedGenres.map((genre) => (
							<Badge variant={artistGenres.includes(genre) ? "default" : "outline"} onClick={(event) => handleGenreClicked(event, genre)} key={genre.id} className="hover:cursor-pointer">{genre.name}</Badge>
						))}
					</div>
				</div>
			</div>
