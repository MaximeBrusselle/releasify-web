...
import { ChooseLabelViewType } from "../../registration/Artist/ChooseLabel";
...
export function ChooseLabel({ label, labelType, updateFields, errors, firebaseLabels }: LabelProps) {
	...
	const handleImageUploaded = () => {
		if (fileInputRef.current?.files && fileInputRef.current.files.length > 0) {
			...
			if (file.size <= 32 * 1024 * 1024) {
				...
				updateFields({ newLabel: { ...createdLabel, profilePicture: URL.createObjectURL(file) } });
				setImageError("");
			} else {
	...
	const viewTypes: ChooseLabelViewType[] = [
		{
			text: "Independant release",
			viewType: "independant",
		},
		{
			text: "Release is on a label",
			viewType: "label",
		},
		{
			text: "Release is on a label not on the platform",
	...
	const [filteredLabels, setFilteredLabels] = useState<LabelDetail[]>(firebaseLabels);

	const [search, setSearch] = useState<string>("");
	function handleSearchChange(event: React.ChangeEvent<HTMLInputElement>) {
		const newValue = event.target.value;
		setSearch(newValue);
		setFilteredLabels(firebaseLabels.filter((label) => label.name.toLowerCase().includes(newValue.toLowerCase())));
	}
					...
					<div className="flex flex-col gap-2 w-full">
						...
						<div className="flex flex-row justify-center items-start flex-wrap gap-x-4 max-h-[50vh] h-fit overflow-y-scroll p-2 w-full gap-y-4">
							{filteredLabels.map((el) => (
								<LabelSelectCard key={el.id} label={el} handleSelect={updateLabel} selectedId={label} />
							))}
						</div>
					</div>
				)}
						...
						<div className="flex flex-col gap-2 w-full">
							...
							<Input
								...
								onChange={(e) => {
									...
									updateFields({ newLabel: { ...createdLabel, name: e.target.value } });
								}}
							/>
