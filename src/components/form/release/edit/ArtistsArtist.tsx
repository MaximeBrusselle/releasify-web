...
type ArtistsArtistView = "solo" | "otherArtists" | "newArtists";
type ArtistsArtistViewType = {
	text: string;
	type: ArtistsArtistView;
};
...
type ArtistsArtistProps = ChooseArtistData & {
	updateFields: (newData: Partial<ChooseArtistData>) => void;
	errors: ValidationFieldErrorMap;
	firebaseArtists: ArtistDetail[];
};

export function ArtistsArtist({ artists, newArtists, updateFields, errors, firebaseArtists }: ArtistsArtistProps) {
	...
	const viewTypes: ArtistsArtistViewType[] = [
		{
			text: "Solo Release",
			type: "solo",
		},
	...
	const [view, setView] = useState<ArtistsArtistViewType>(viewTypes[1]);

	function updateViewType(event: React.MouseEvent<HTMLButtonElement, MouseEvent>, index: number) {
		event.preventDefault();
        const newViewType = viewTypes[index].type;
		if (view.type === newViewType) return;
        if(newViewType === "solo") {
            updateFields({ artists: [] });
            updateFields({ newArtists: [] });
        }
		setView(viewTypes[index]);
	}

	const [filteredOtherArtists, setFilteredOtherArtists] = useState<ArtistDetail[]>(firebaseArtists.filter((artist) => artist.id !== objectId));
					...
					<div className="flex flex-col gap-2 w-full">
						<div className="flex flex-row justify-start items-center m-0 p-0">
							<Label htmlFor="existingartists" className="font-bold text-lg">
								No need to do anything. This is a solo release.
							</Label>
						</div>
					</div>
				)}
					...
					<div className="flex flex-col gap-2 w-full">
						...
						<div className="grid lg:grid-cols-3 sm:grid-cols-2 gap-x-4 max-h-[50vh] h-fit overflow-y-scroll p-2 w-full gap-y-4 place-items-center">
							...
							{filteredOtherArtists.length > 0 &&
								filteredOtherArtists.map((el) => <ArtistSelectCard key={el.id} artist={el} handleSelect={handleArtistSelect} selectedIds={artists.map((x) => x.id)} />)}
						</div>
					</div>
				)}
