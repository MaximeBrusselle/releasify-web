					...
					<div className="mx-auto max-w-7xl px-2 sm:px-6 lg:px-8">
						<div className="relative flex h-16 items-center justify-between">
							<div className="absolute inset-y-0 left-0 flex items-center sm:hidden">
								{/* Mobile menu button*/}
								<Disclosure.Button className="relative inline-flex items-center justify-center rounded-md p-2 text-text hover:bg-primary hover:text-primary focus:outline-none focus:ring-2 focus:ring-inset focus:ring-text">
								...
								<Menu as="div" className="relative ml-3">
									...
									>
										<Menu.Items className="absolute right-0 z-10 mt-2 w-48 origin-top-right rounded-md bg-background py-1 shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none">
											...
											<Menu.Item>
												{({ active }) => (
													<a href="/sign-out" className={classNames(active ? "bg-primary text-primary hover:text-[#f00]" : "", "block px-4 py-2 text-sm text-[#f00]")}>
														Sign out
													</a>
												)}
											</Menu.Item>
