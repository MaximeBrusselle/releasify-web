...
const ArtistCard: React.FC<ArtistCardProps> = (props: ArtistCardProps) => {
	...
	const [following, setFollowing] = useState(userData.following.some((followedArtist: string) => followedArtist === artist.id));
	const [notification, setNotification] = useState(userData.notifications.some((notifiedArtist: string) => notifiedArtist === artist.id));

	const handleFollow = async (e: React.MouseEvent<HTMLButtonElement>) => {
		e.stopPropagation();
		const result = await followArtist(artist.id!, following);
	...
	const handleNotification = async (e: React.MouseEvent<HTMLButtonElement>) => {
		e.stopPropagation();
		const result = await notificationsArtist(artist.id!, following);
		if (result.code === "success") {
			toast.success(result.message);
		} else {
			toast.error(result.message);
			...
			<div className="flex sm:flex-row flex-col items-center sm:justify-start justify-center w-full h-full gap-4">
				<div className="flex flex-row sm:justify-start justify-center items-center gap-2 w-full">
					...
					<div className="flex flex-col items-start justify-center h-full">
						...
						{artist.id && !artist.id.startsWith("notExists") && (
