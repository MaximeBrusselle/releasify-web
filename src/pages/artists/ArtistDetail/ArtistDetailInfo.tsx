		...
		<div className="flex flex-col items-center justify-start gap-[14px] w-[75vw]">
				...
				<div className="flex flex-row w-full h-full gap-[14px] items-center justify-center border-solid">
					...
					<img
						...
						alt="profile picture"
						className="w-[240px] object-cover rounded-[50px] border-black border-[6px] border-solid aspect-square"
					/>
					{/* Info */}
					<div className="flex flex-row items-center justify-between h-[200px] w-full gap-6">
							...
							<div className="flex flex-col items-start justify-center gap-0">
								...
								{artist.bookingEmail && (
									<p className="text-[16px] flex flex-row gap-1">
										Booking:{" "}
										<a className="text-[16px] text-blue-600 underline" href={`mailto:${artist.bookingEmail}?subject=Booking ${artist.artistName}`}>
											{artist.bookingEmail}
										</a>
									</p>
								)}
						...
						<div className="flex flex-col justify-start items-center w-full h-full">
							...
							<div className="grid grid-cols-2 w-full gap-y-[14px]">
								{artist.socials.map((social) => (
									<a href={social.url} key={social.platform.name}>
						...
						<div className="flex flex-col justify-center items-center w-full h-full">
							...
							{artist.label && (
								<a href={`/labels/${artist.label.id}`}>
									<div className="flex flex-row items-center justify-center gap-3">
										<img src={getImageUrl("labels", artist.label.picture)} alt={artist.label.name} className="w-20 aspect-square rounded-md" />
